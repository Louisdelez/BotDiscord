...
export function WatchTogetherPlayer() {
  ...
  const {
    channelId, joined, currentVideo, queue, currentIndex, playing,
    startedAt, pausedAt, volume, listeners,
    init, cleanup, joinChannel, leaveChannel, addVideo, skip, pause, resume, seek, setVolume, getTargetPosition,
  } = store;
  ...
  useEffect(() => {
    if (!guildId) return;
    api.get(`/guilds/${guildId}/voice-channels`).then(d => setVoiceChannels(Array.isArray(d) ? d : [])).catch(() => {});
  }, [guildId]);
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-[var(--text-secondary)]">
          {t('web.watchTogether.selectChannel')}
        </p>
        {voiceChannels.length === 0 ? (
          <p className="text-center py-8 text-[var(--text-secondary)]">
            {t('web.watchTogether.noVoiceChannel')}
          </p>
        ) : (
  ...
  return (
    <div className="space-y-5">
      {/* Video Player — visible, 16:9 responsive */}
      ...
      {currentVideo ? (
        <div className="p-4 rounded-xl bg-[var(--bg-secondary)]">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-[var(--text-secondary)]">{playing ? t('web.watchTogether.playing') : t('web.watchTogether.paused')}</span>
          </div>
          <p className="font-semibold text-lg truncate">{currentVideo.title || currentVideo.url}</p>
        ...
        <div className="text-center py-8 text-[var(--text-secondary)]">
          <p>{t('web.watchTogether.noVideo')}</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <Button onClick={() => leaveChannel()} variant="danger" size="md">
          <LogOut size={18} /> {t('web.watchTogether.leave')}
        </Button>
      </div>
      ...
      <div className="flex gap-2">
        <Input
          ...
          placeholder={t('web.watchTogether.urlPlaceholder')}
      ...
      {queue.length > 0 && (
        <div>
          <p className="text-sm font-medium mb-2">{t('web.watchTogether.queue', { count: queue.length })}</p>
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {queue.map((video, i) => (
              ...
              >
                ...
                <p className="text-sm truncate flex-1">{video.title || video.url}</p>
              </div>
            ))}
          </div>
        </div>

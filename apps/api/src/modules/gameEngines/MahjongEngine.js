...
class MahjongEngine {
  ...
  serialize(forPlayerIndex = null) {
    return {
      ...
      melds: this.players.map(p => p.exposed),
      wallRemaining: this.wall.length,
      gameOver: this.phase === 'gameOver',
      winner: this.winner,
      // Private info - only show requesting player's hand
      hands: this.players.map((p, i) =>
        i === forPlayerIndex ? p.hand : { count: p.hand.length }
      ),
      playerCount: 4,
    };
  }
...
module.exports = { MahjongEngine, tilesMatch, tileKey, sortTiles, isNumeric, isWinningHand, SUITS, WINDS, DRAGONS };

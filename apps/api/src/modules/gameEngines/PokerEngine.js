...
class PokerEngine {
  ...
  // --- Serialization (information hiding) ---

  serialize(forPlayerIndex = null) {
    const isShowdown = this.showdown || this.handOver;

    return {
      phase: this.bettingRound,
      pot: this.pot,
      ...
      players: this.players.map((p, i) => ({
        ...
        cards: i === forPlayerIndex || isShowdown
          ? deepClone(p.holeCards)
          : (p.folded ? [] : p.holeCards.length > 0 ? [null, null] : []),
      })),
...
module.exports = { PokerEngine, SUITS, SUIT_SYMBOLS, VALUES, HAND_NAMES, evaluateBestHand, compareHands };
